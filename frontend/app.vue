...
<template>
    ...
    <div class="h-screen flex flex-col">
        <div class="navbar bg-base-100">
            <div class="flex-1">
                <NuxtLink to="/">
                    <div class="btn btn-ghost gap-1">
                        ...
                        <div
                            class="badge badge-warning bg-amber-500 m-0 normal-case text-lg rounded-sm font-medium p-0">
                            Hub
                        </div>
                    </div>
                </NuxtLink>
                <button @click="toggleMode" class="btn btn-sm btn-ghost ml-auto mr-2" aria-label="Theme switch">
                    <svg v-if="colorMode.preference === 'dark'" class="swap-on fill-current w-5 h-5"
                    ...
                    <svg v-else class="swap-off fill-current w-5 h-5" xmlns="http://www.w3.org/2000/svg"
                         viewBox="0 0 24 24">
                        <path
            ...
            <div v-if="status === 'authenticated'" class="flex-none">
                <div class="dropdown dropdown-end">
                    ...
                    <ul tabIndex="0"
                        class="mt-3 z-[900] p-2 shadow menu menu-sm dropdown-content bg-base-100 rounded-box w-52">
                        <li>
                            <NuxtLink :to="`/profile`">Profile</NuxtLink>
                        </li>
                        <li v-if="data.role && data.role === 'ADMIN'">
        ...
        <div class="fixed bottom-4 right-4 lg:hidden z-50">
            <button class="rounded-full bg-accent text-white p-2" @click="toggleSidebar()" aria-label="sidebar">
                <svg
                    class="h-6 w-6"
                    viewBox="0 0 24 24"
                    fill="none"
